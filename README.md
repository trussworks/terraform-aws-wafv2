...
| filtered\_header\_rule | List of headers to filter by | <pre>object({<br>    names         = list(string)<br>    priority      = number<br>    filter_header = string<br>  })</pre> | <pre>{<br>  "filter_header": "",<br>  "names": [],<br>  "priority": 1<br>}</pre> | no |
...
| rate\_based\_rule | List of rate based rules to apply | <pre>object({<br>    name     = string<br>    priority = number<br>    limit    = number<br>    action   = string<br>  })</pre> | `null` | no |
